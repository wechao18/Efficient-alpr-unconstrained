...
class AELoss2(nn.Module):
    ...
    def forward(self, outs, targets):
        ...
        gt_ct_heat = targets[0]
        gt_mask    = targets[1]
        gt_ct_regr = targets[2]
        gt_ct_wh = targets[3]
        gt_point = targets[4]
        ...
        try :
            focal_loss += self.focal_loss(ct_heat, gt_ct_heat)
        except:
            print(1)
        ...
        scale_loss = self.reg_wh(ct_wh, gt_ct_wh, gt_mask)
        scale_loss = self.wh_weight * scale_loss
        ...
        point_loss = self.reg_point(lpr_point, gt_point, gt_mask)
        point_loss = 1 * point_loss

        loss = (focal_loss + regr_loss + scale_loss + point_loss) / len(ct_heat)
        #, regr_loss, scale_loss, point_loss
        return loss.unsqueeze(0), focal_loss, regr_loss, scale_loss, point_loss
