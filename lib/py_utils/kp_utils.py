...
def decode_lpr(
        ...
        K=1, kernel=1, ae_threshold=1, num_dets=1
):
    ...
    lpr_point = torch.ones((ct_regr.shape[0], ct_regr.shape[1], 3 * 4)).cuda()
...
class RegL1Loss1(nn.Module):
    ...
    def forward(self, pred, target, mask):
        mask = mask.unsqueeze(2).expand_as(pred).float()
        # loss = F.l1_loss(pred * mask, target * mask, reduction='elementwise_mean')
        mask_gt = torch.gt(pred, target)
        mask_le = torch.le(pred, target)
        #loss = nn.functional.l1_loss(pred * mask, target * mask, size_average=False)
        loss_le = 2 * nn.functional.l1_loss(pred[mask_le], target[mask_le], size_average=False)
        loss_gt = nn.functional.l1_loss(pred[mask_gt], target[mask_gt], size_average=False)
        #loss = loss / (mask.sum() + 1e-4)
...
class RegL1Loss2(nn.Module):
    ...
    def forward(self, pred, target, mask):
        ...
        loss_le1 = nn.functional.l1_loss(pred_le[mask_le1], target_le[mask_le1], size_average=False)
        loss_gt1 = 2 * nn.functional.l1_loss(pred_le[mask_gt1], target_le[mask_gt1], size_average=False)

        loss_le2 = 2 * nn.functional.l1_loss(pred_gt[mask_le2], target_gt[mask_le2], size_average=False)
        loss_gt2 = nn.functional.l1_loss(pred_gt[mask_gt2], target_gt[mask_gt2], size_average=False)
...
class RegMSELoss(nn.Module):
    ...
    def forward(self, pred, target, mask):
        mask = mask.unsqueeze(2).expand_as(pred).float()
        # loss = F.l1_loss(pred * mask, target * mask, reduction='elementwise_mean')
        loss = nn.functional.mse_loss(pred * mask, target * mask, size_average=False)
        loss = loss / (mask.sum() + 1e-4)
        return loss